...
import {
  ...
  SiGit, SiGithub, SiNpm, SiVite, SiNodedotjs, SiExpress, SiMongodb,
  SiDocker, SiFirebase, SiVercel
} from "react-icons/si";
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      stars.forEach((star) => {
        ...
        ctx.fillStyle = `rgba(255, 215, 0, ${Math.max(0, star.alpha * 0.4)})`;
        ctx.beginPath();
    ...
    >
      ...
      <div className="container-page relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          >
            My Tech <span className="text-gradient-brand">Universe</span>
          </motion.h2>
          ...
          >
            Explore my full-stack ecosystem. Three interactive orbits covering frontend, backend, and cloud tools. Hover or tap any planet to inspect skill metrics.
          </motion.p>
          ...
          >
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-extrabold tracking-wider uppercase transition-all duration-300 ${
                activeOrbitId === null
                  ? "bg-gradient-to-r from-[#FFD700] to-[#FFD700] text-ink-900 shadow-[0_4px_20px_rgba(255,215,0,0.4)]"
                  : "glass text-white hover:text-[#FFD700] border border-white/10"
              }`}
            >
              All Orbits
            </button>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-extrabold tracking-wider uppercase transition-all duration-300 ${
                  activeOrbitId === orbit.id
                    ? "bg-[#FFD700] text-ink-900 font-bold shadow-[0_4px_20px_rgba(255,215,0,0.5)]"
                    : "glass text-white hover:text-[#FFD700] border border-white/10"
                }`}
              >
        ...
        <div className="relative w-full max-w-6xl mx-auto h-[620px] sm:h-[720px] flex items-center justify-center perspective-[1200px]">
          ...
          >
            ...
            >
              <div className="absolute w-44 h-44 sm:w-56 sm:h-56 rounded-full bg-gradient-to-r from-[#FFD700]/30 via-[#FFD700]/20 to-transparent blur-2xl animate-pulse" />
              <div className="absolute w-36 h-36 sm:w-44 sm:h-44 rounded-full bg-gradient-to-tr from-[#FFD700]/50 to-[#FFD700]/30 blur-xl animate-spin-slow" />

              <div className="absolute w-48 h-48 sm:w-60 sm:h-60 rounded-full border border-[#FFD700]/40 animate-ping opacity-30" />
              <div className="absolute w-56 h-56 sm:w-68 sm:h-68 rounded-full border border-[#FFD700]/30 animate-spin-slow" />

              <div className="relative w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-gradient-to-br from-[#FFD700] via-[#ffffff] to-[#FFD700] shadow-[0_0_60px_rgba(255,215,0,0.8)] border-2 border-white flex flex-col items-center justify-center p-3 text-center transition-transform duration-500 group-hover:scale-110">
                <span className="text-xl sm:text-2xl mb-0.5 animate-bounce text-ink-900">☀</span>
                <span className="text-[0.65rem] sm:text-xs font-black tracking-wider text-ink-900 uppercase leading-tight">
                  Full-Stack
                </span>
                <span className="text-[0.6rem] sm:text-[0.7rem] font-extrabold text-ink-900 tracking-widest uppercase">
                  Core
                </span>
              </div>

              <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 whitespace-nowrap bg-ink-900/90 backdrop-blur-md border border-[#FFD700]/50 px-3.5 py-1 rounded-full text-[0.65rem] font-extrabold text-[#FFD700] tracking-widest uppercase shadow-lg">
                React · Node · Cloud
              </div>
            </div>
              ...
              return (
                <div
                  ...
                  className={`absolute rounded-full border transition-all duration-500 pointer-events-none ${
                    isHighlighted
                      ? "border-[#FFD700] shadow-[0_0_30px_rgba(255,215,0,0.5)] border-2"
                      : isDimmed
                ...
                >
                  <div
                    className="absolute -top-3 left-1/2 -translate-x-1/2 text-[0.6rem] font-bold tracking-widest text-[#FFD700] uppercase bg-ink-900/90 px-2.5 py-0.5 rounded-full border border-[#FFD700]/30 shadow-sm"
          ...
          <AnimatePresence>
            {activePlanet && (
              <motion.div
                ...
                className="absolute bottom-4 left-1/2 -translate-x-1/2 z-40 w-full max-w-md p-5 rounded-2xl glass-strong border border-[#FFD700]/20 shadow-[0_16px_50px_rgba(0,0,0,0.5)] text-white"
              >
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-[0.65rem] uppercase tracking-widest text-[#FFD700] font-extrabold">
                        Skill Proficiency
                      </span>
                    </div>
                  </div>
                  <button
                    ...
                    className="text-white/60 hover:text-white text-xs px-2 py-1 rounded-full bg-white/5 hover:bg-white/10"
                  >
                    ✕
                  </button>
                </div>
                ...
                <p className="text-xs text-white/70 font-medium leading-relaxed mb-3">
                  {activePlanet.desc}
                </p>

                <div className="flex justify-between items-center text-[0.65rem] text-muted font-semibold pt-2 border-t border-white/5">
                  <span>Full-Stack Ecosystem Node</span>
                  <span className="text-[#FFD700]">Interactive Orbit Active</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {SOLAR_ORBITS.map((orbit) => (
            <motion.div
              ...
              className={`cursor-pointer rounded-2xl border p-6 glass transition-all duration-300 ${
                activeOrbitId === orbit.id
                  ? "border-[#FFD700] shadow-[0_10px_30px_rgba(255,215,0,0.2)] bg-white/5"
                  : "border-white/10 hover:border-[#FFD700]"
              }`}
            >
              <div className="flex items-center gap-3 mb-4">
                <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-[#FFD700]/10 text-[#FFD700] font-bold text-xs border border-[#FFD700]/30">
                  0{orbit.id}
                </span>
              ...
              <div className="flex flex-wrap gap-1.5">
                {orbit.planets.map((p) => (
                  <span
                    key={p.name}
                    className="text-[0.65rem] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border border-white/10 text-white/80 bg-white/5 shadow-xs"
                  >
                    {p.name}
                  </span>
                ))}
      ...
      <div
        className={`absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap px-2 py-0.5 rounded-full text-[0.6rem] font-bold tracking-wider uppercase transition-all duration-300 pointer-events-none ${
          isHovered || isSelected
            ? "bg-ink-900 text-white border border-[#FFD700] opacity-100 scale-100 shadow-md"
            : "bg-ink-900/80 text-white/80 opacity-0 group-hover:opacity-100 scale-90"
        }`}
      >
        {planet.name}
      </div>
